...
# Funzione per rimuovere il prefisso 'chr', se presente
...
def main(bed1, bed2, ancestry0, ancestry1, ancestry2, ancestry3, ancestry4, unknown, start, end, chromosome, feature_type, output):
    ...
    for line in bed1:
        ...
        line[0] = remove_chr_prefix(line[0])  # Rimuovi il prefisso 'chr', se presente


        bedLine = f"{line[0]}\t{line[1]}\t{line[2]}\tgeom_rect\t{colors[line[3]]}\t1"
    ...
    for line in bed2:
        ...
        bedLine = f"{line[0]}\t{line[1]}\t{line[2]}\tgeom_rect\t{colors[line[3]]}\t2"
        output_lines.append(bedLine)
    ...
    if start is not None and end is not None and chromosome is not None:
        if feature_type == 'line':
            extra_lines = [
                f"{chromosome}\t{start}\t{end}\tgeom_line\t#000000\t1",
                f"{chromosome}\t{start}\t{end}\tgeom_line\t#000000\t2"
            ]
        else:
            extra_lines = [
                f"{chromosome}\t{start}\t{end}\tgeom_line\t#000000\t1",
